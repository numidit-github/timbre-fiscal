...
class KsGlobalTaxInvoice(models.Model):
    ...
    ks_global_tax_rate = fields.Float(string='Timbre Fiscal (%):', readonly=True, default=1.0,
                                      states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    ks_amount_global_tax = fields.Monetary(string="Timbre Fiscal", readonly=True, compute='_compute_amount',
    ...
    @api.onchange('payment_mode_id', 'partner_id')
    ...
    def _compute_amount(self):
        ...
        for rec in self:
            ...
            rec.ks_update_timbre_fiscal_dz_15()
    ...
    def ks_calculate_tax(self):
        for rec in self:
            ...
            if rec.is_payment_mode_tax_stamp and rec.move_type in type_list:
                rec.ks_amount_global_tax = (
                    rec.amount_total * rec.ks_global_tax_rate) / 100
                if rec.ks_amount_global_tax > 2500:
                    rec.ks_amount_global_tax = 2500
            else:
            ...
            if rec.is_payment_mode_tax_stamp and rec.move_type in type_list:
                if rec.is_invoice(include_receipts=True):
                    ...
                    if in_draft_mode:
                        ...
                        if terms_lines:
                            self.line_ids = [
                                (1, already_exists.id, dict1), (1, terms_lines[0].id, dict2)]
